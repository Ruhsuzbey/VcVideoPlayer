...
async def stream(client, m: Message):
    ...
    if not replied:
        if len(m.command) < 2:
            await m.reply("`Bazı Video Dosyasını Yanıtlama!`")
        else:
            ...
            if match:
                ...
                livelink = query
                photoid = "https://telegra.ph/file/5ebe00115202ef9c4d873.jpg"
                msg = await m.reply_photo(photo=photoid, caption="`Video Akışı Başlatılıyor🤫...`")
            ...
            try:
                ...
                await msg.edit_caption(f"**Başladı🥳 [Video Stream]({livelink}) !**")
                await idle()
            except Exception as e:
        ...
        if replied.video.thumbs:
            ...
            photoid = "https://telegra.ph/file/5ebe00115202ef9c4d873.jpg"
        msg = await m.reply_photo(photo=photoid, caption="`indiriyor...😴`")
        video = await client.download_media(m.reply_to_message)
        chat_id = m.chat.id
        await msg.edit_caption("`işleniyor...🤗`")
        ...
        try:
            ...
            await msg.edit_caption("**Başlatılan Video Akışı!🤭**")
        except Exception as e:
            await msg.edit_caption(f"**Error** -- `{e}`")
            await idle()
    else:
        await m.reply("`Bazı Videoyu Yanıtlama!`")
...
async def stopvideo(client, m: Message):
    ...
    try:
        ...
        await m.reply("**⏹️ Video Akışını Durdur🥵!**")
    except Exception as e:
    ...
    if not replied:
        if len(m.command) < 2:
            ...
            if match:
                try:
                    ...
                    for f in formats:
                        ytstreamlink = f['url']
                    livelink = ytstreamlink
                    msg = await m.reply("`YT Akışı Başlatılıyor...😴`")
                except Exception as e:
                    msg = await m.reply(f"{e}")
                    return
            else:
            ...
            try:
                ...
                await msg.edit("**Başlatılan Kanal Akışı!**")
                await idle()
            except Exception as e:
                await msg.edit(f"**Error** -- `{e}`")
   
    elif replied.video or replied.document:
        msg = await m.reply("`indiriyor...😴`")
        video = await client.download_media(m.reply_to_message)
        chat_id = CHANNEL
        await msg.edit("`işleniyor 🤗🧬...`")
        os.system(f"ffmpeg -i '{video}' -f s16le -ac 1 -ar 48000 'audio{chat_id}.raw' -y -f rawvideo -r 20 -pix_fmt yuv420p -vf scale=640:360 'video{chat_id}.raw' -y")
        try:
            audio_file = f'audio{chat_id}.raw'
            video_file = f'video{chat_id}.raw'
    ...
    try:
        ...
        if process:
            try:
                ...
                print(e)
        await call_py.leave_group_call(chat_id)
        await m.reply("**⏹️ Kanal Akışını Durdur!**")
    except Exception as e:
        await m.reply(f"**🚫 Error** - `{e}`")
